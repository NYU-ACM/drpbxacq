...
case class File(id: UUID, filename: String, path: String, humanSize: String, size: Long, modDate: java.sql.Date, status: String)
...
class Files(tag: Tag) extends Table[File](tag, "FILES") {
  ...
  def * = (id, filename, path, humanSize, size, modDate, status) <> (File.tupled, File.unapply _)
}
